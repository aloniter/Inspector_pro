...
@Test func docxTemplateDefinesRTLNumberingPartRelationshipAndBulletStyle() {
    ...
    // Attendees no longer use Word auto-numbering: numId 2 / abstractNumId 2
    // and the paragraph style that referenced them must not exist.
...
@Test func docxCoverDetailsAvoidsDirectionalIsolatesAndUsesSeparateLabelValueParagraphs() throws {
    ...
    let firstAttendeeMarkerParagraph = try #require(xml.components(separatedBy: "<w:p>").first { $0.contains(">1.<") })
    let secondAttendeeParagraph = try #require(xml.components(separatedBy: "<w:p>").first { $0.contains(">דפנה<") })
    let secondAttendeeMarkerParagraph = try #require(xml.components(separatedBy: "<w:p>").first { $0.contains(">2.<") })
    ...
    #expect(xml.contains("<w:gridCol w:w=\"680\"/>"))
    ...
    // Name cell: plain left-aligned text, no auto-numbering, no bidi flip.
    ...
    // Marker cell: literal "1." text, right-aligned, no bidi/rtl tag needed
    // for plain Western digits.
    #expect(firstAttendeeMarkerParagraph.contains("w:jc w:val=\"right\""))
    #expect(!firstAttendeeMarkerParagraph.contains("<w:bidi/>"))
    #expect(!firstAttendeeMarkerParagraph.contains("<w:rtl/>"))
    #expect(!firstAttendeeMarkerParagraph.contains("<w:numPr>"))
    #expect(secondAttendeeParagraph.contains("w:jc w:val=\"left\""))
    #expect(secondAttendeeMarkerParagraph.contains(">2.<"))
...
@Test func docxExporterGeneratesCoverWithRTLAttendeeMarkers() async throws {
    ...
    #expect(documentText.contains("<w:gridCol w:w=\"680\"/>"))
